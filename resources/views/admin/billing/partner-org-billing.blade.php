                        ...
                        <div class="col-md-12">
                            <div class="card custom-card">
                                <div class="card-header">
                                    <h4 class="card-title"><i class="far fa-newspaper"></i> Partner Organisation Billing</h4>
                                ...
                                <div class="card-body">
                                    <form action="{{url('superadmin/partnerorg/bills/store')}}" method="post" enctype="multipart/form-data">
                                        {{csrf_field()}}
                                        <div class="row form-group">
                                            ...
                                            <div class="col-md-3">
                                                <div class="form-group">
                                                    <label for="billing_type" class="placeholder">Partner</label>
                                                    <select class="form-control input-border-bottom" id="billing_type" name="billing_type" required="" style="margin-top: 22px;" onchange="getBillingEntities(this.value);">
                                                        ...
                                                        <option value="{{ $partner->org_code }}"> {{ $partner->com_name }} </option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                            </div>
                                            ...
                                            <div class="col-md-3" id="entity_dropdown">
                                                <div class="form-group">
                                                    <label for="entity_id" class="placeholder">Select Partner Organisation</label>
    ...
    <script>
        $(document).ready(function() {
            $('#billing_type').change(function() {
                let billingTypeId = $(this).val();
    
                if (billingTypeId) {
                    $.ajax({
                        url: "{{ url('superadmin/get-partner-organizations') }}",
                        type: "GET",
                        data: { billing_type: billingTypeId },
                        success: function(response) {
                            console.log(response);
                            $('#entity_id').empty().append('<option value="">Select Organisation</option>');
    
                            $.each(response, function(index, organization) {
                                $('#entity_id').append(
                                    `<option value="${organization.reg}">${organization.com_name}</option>`
                                );
                            });
                        }
                    });
        ...
        function getUserDetails(userId) {
            if (userId != '') {
                ...
                //alert(userId);
                $.ajax({
                    url: "{{ route('get.user.dtl') }}", // The route to call
                    type: 'GET',
                    data: { user_id: userId,
