...
@section('title', \App\Helpers\Helper::cachedTrans('Leave Policy'))
@php 
...
<div class="content container-fluid pb-0">
	...
	<div class="page-header">
		<div class="row align-items-center">
			<div class="col">
				<h3 class="page-title">{{\App\Helpers\Helper::cachedTrans('Leave Policy')}}</h3>
				<ul class="breadcrumb">
					<li class="breadcrumb-item"><a href="{{url('organization/employerdashboard')}}">{{\App\Helpers\Helper::cachedTrans('Home')}}</a></li>
                    <li class="breadcrumb-item"><a href="{{url('leave/dashboard')}}">{{\App\Helpers\Helper::cachedTrans('Dashboard')}}</a></li>
					<li class="breadcrumb-item active">{{\App\Helpers\Helper::cachedTrans('Policy')}}</li>
				</ul>
			</div>
				...
				@if($user_type == 'employee')
				    @foreach($sidebarItems['Leave Management'] as $rotaItem)
                        @if($rotaItem['submenu_name'] == 'Policy' && $rotaItem['can_add'] == 1)
				            <a href="{{ url('leave/save-leave-rule') }}" class="btn add-btn"><i class="fa-solid fa-plus"></i> {{\App\Helpers\Helper::cachedTrans('Add Policy')}}</a>
				        @endif
				    @endforeach
		...
		<div class="col-md-12">
            <div class="card custom-card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h4 class="card-title">
                        <i class="far fa-file" aria-hidden="true" style="color:#ffa318;"></i>&nbsp; {{\App\Helpers\Helper::cachedTrans('Policy')}}
                    </h4>
                    <div class="row">
                        <div class="col-auto">
                            <form action="{{ route('exportTableData') }}" method="POST" id="exportForm" class="d-inline">
                                ...
                                <input type="hidden" id="filenameInput" value="Leave Policy">
                        ...
                        <table class="table table-striped custom-table" id="basic-datatables">
                            <thead>
                                <tr>
                                    ...
                                    <th>{{\App\Helpers\Helper::cachedTrans('Max. No.')}} </th>
                                    <th>{{\App\Helpers\Helper::cachedTrans('Effective From')}} </th>
                                    <th>{{\App\Helpers\Helper::cachedTrans('Effective To')}} </th>
