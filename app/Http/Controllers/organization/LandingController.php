...
class LandingController extends Controller
{
    ...
    public function employeeDashboard(Request $request){
        if (!empty(Session::get('emp_email'))) {
            ...
            $data["designation_count"] = DB::table('designation')->where('emid', '=', $Roledata->reg)->count();
            $data['employee_type_count'] = DB::Table('employee_type')->where('emid', '=', $Roledata->reg)->count();
            $data["shift_count"] = ShiftManagment::where("emid", "=", $Roledata->reg)->count();
            
            return view('employeer/employee/dashboard',$data);
        }else{
            return redirect('/');
        }
    }
    ...
    //                 ->where('users.status', '=', 'active')
    //                 ->where('users.user_type', '=', 'employee')
    //                 ->orderBy('users.id', 'asc') // or any other column you want to sort by
    ...
    public function employeeActive(Request $request)
    {
        ...
        // Check if the ID exists in both tables
        $userExists = DB::table('users')->where('employee_id', $decrypted_id)->exists();
        $employeeExists = DB::table('employee')->where('emp_code', $decrypted_id)->exists();
        // If both records exist, update their status
        if ($userExists && $employeeExists) {
            ...
            DB::table('employee')
                ...
                ->update(['status' => 'active','emp_status'=>'FULL TIME']);

            Session::flash("message", "Employee active successfully!");
            return redirect()->back();
        } else {
            return redirect()->back()->with('error', 'Employee not found in the database.');
        }
    }
    ...
    public function employeeInactive(Request $request)
    {
        ...
        if ($userExists && $employeeExists) {
            ...
            DB::table('employee')
                ->where('emp_code', $decrypted_id)
                ->update(['status' => 'inactive','emp_status'=>'LEFT','updated_at'=> now()]);

            Session::flash("message", "Employee inactive successfully!");
            return redirect()->back();
        } else {
            return redirect()->back()->with('error', 'Employee not found in the database.');
        }
    ...
    public function saveGuest(Request $request)
    {
        ...
        DB::transaction(function () use ($request, $reg) {
            ...
            if ($request->id) {
                ...
                $data = [
                    'org_name'        => $organization->com_name ?? '',
                ...
                Mail::send(
                    'organization-email-template.guest-creadential-email',
                    $data,
