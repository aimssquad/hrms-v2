...
class RecruitmentController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->url          = "https://skilledworkerscloud.co.uk/hrms-v2/";
    }
        ...
        if (!empty(Session::get('emp_email'))) {
            ...
            if(Input::get('id') ==''){
                ...
                if (count($dt) > 0) {
                    ...
                    return redirect('org-recruitment/add-job-list');
                }
            }
    ...
    public function saveJobPostData(Request $request)
    {
        ...
        if (!empty(Session::get('emp_email'))) {
            ...
            if (Input::get('id')) {$ckeck_dept = DB::table('company_job')->where('soc',$request->soc)->where('id', '!=', Input::get('id'))->where('emid', $reg)->first();
               ...
               $arrayvalu=[
                "job_link"=>$this->url . 'career/' . base64_encode(($isertId)),
               ];
            ...
            if ($data['candidate_rs']) {$f = 1;
                foreach ($data['candidate_rs'] as $leave_allocation) {$job_details = DB::table('candidate_history')->where('user_id', '=', $leave_allocation->id)->orderBy('id', 'DESC')->first();
                    ...
                    $data['result'] .= '<tr>
											 ...
											 <td>
                                <a href="' . asset('public/'.$leave_allocation->resume) . '" download>
        ...
        if (!empty($email)) {
            ...
            if ($request->hasFile('photos')) {
                ...
                foreach ($candidate as $photoll) {
                    ...
                    $path[] = Storage::disk('public')->path($photoll->file);
                }
                //dd($path);
                $sub = $request->subject;
    ...
    public function viewsendcandidatedetailsjobshorting($send_id)
    {
        if (!empty(Session::get('emp_email'))) {
            ...
            $dataup = array('name' => $job->name, 'pos' => $job->job_title, 'job_code' => $job_d->soc, 'status' => $job->status,
    ...
    public function viewhiredcandidatedetails($hired_id)
    {
        ...
        if (!empty(Session::get('emp_email'))) {
            ...
            //dd($data);
            return view($this->_routePrefix . '.hired-edit',$data);
    ...
    public function getsearchcandidatestatus(Request $request)
    {
        ...
        if (!empty(Session::get('emp_email'))) {
                ...
                foreach ($data['candidate_rs'] as $leave_allocation) {
                    ...
                    if (!empty($job_details)) {
                        ...
                        $data['result'] .= '<tr>
													...
													<td>' . $end . '</td>
															<td><a href="' . $this->url . 'recruitment/apply-letter/' . base64_encode($leave_allocation->id) . '" target="_blank" ><i class="fas fa-paper-plane"></i></a></td>

									<td>
									<a href="' . $this->url . 'public/' . $leave_allocation->resume . '" download><i class="fas fa-download"></i></a>	</td>
                        ...
                        foreach ($job_ff as $leave_allocationjj) {
                            ...
                            if ($leave_allocationjj->status == 'Interview') {
                                $url = '<a href="' . $this->url . 'recruitment/interview-letter/' . base64_encode($leave_allocation->id) . '" target="_blank" ><i class="fas fa-paper-plane"></i></a>';
                            } else {
                                $url = '';
                            }
            ...
            $data['end_date'] = $request->end_date;
            return view($this->_routePrefix . '.search-status',$data);
            //return view('org-recruitment/status-search', $data);
        } else {
            return redirect('/');
        }
    }
