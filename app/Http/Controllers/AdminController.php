...
<img style="width: 19px;" src="https://skilledworkerscloud.co.uk/hrms/public/assets/img/download.png"></a>';} else {
    ...
    public function viewSubChildCompany($comp_id)
    {
        //dd($comp_id);
           try {
            $email = Session::get('empsu_email');
    ...
    public function saveSubChildCompany(Request $request)
    {
        //dd($request->all());
        //dd($request->reg);
        try {
    ...
    public function saveSubCompany(Request $request)
    {
        ...
        //dd($request->all());
            ...
            if (!empty($email)) {
                ...
                if ($request->status == 'active' && $request->verify == 'approved' && $request->licence == 'no') {
                    $org_exist = DB::table('sub_admin_registrations')->where('reg', $request->reg)->first();
                    if($org_exist->org_code){
                         $data = array(
                        'status' => $request->status,
                        'verify' => $request->verify,
                        'employee_link' => env("BASE_URL") . 'new-employee/' . base64_encode($request->reg),
                ...
                if ($request->status == 'active' && $request->verify == 'not approved' && $request->licence == 'no') {
                    ...
                    return redirect('superadmin/notverify');
                } else if ($request->status == 'inactive' && $request->verify == 'not approved' && $request->licence == 'no') {
                    //return redirect('superadmin/inactive');
                    return redirect('superadmindasboard');
                } else if ($request->status == 'inactive' && $request->verify == 'approved' && $request->licence == 'no') {
    ...
    public function view_login_file_dtl(Request $request)
    {   
        if (!empty(Session::get('empsu_email'))) {
            //dd('okk');
            // Get the latest video (change this logic as per your needs)
            $videos="";
            $videos = DB::table('login_page_image')->get();
        
            if ($videos->isNotEmpty()) {
                // Pass all videos to the view
                return view('admin/login_file', compact('videos'));
            } else {
                // Handle case when no video is found
                Session::flash('message', 'No videos found');
                return view('admin/login_file',compact('videos'));
            }
            //return view('admin/login_file');
        } else {
            return redirect('superadmin');
        }
    }

    public function view_login_file_form(Request $request)
    {
        if (!empty(Session::get('empsu_email'))) {
            return view('admin/login_file_form');
        } else{
            return redirect('superadmin');
        }  
    }
    ...
    //         'image' => 'required|file|mimes:jpeg,jpg,png,gif,mp4|max:51200', // Adjust the mime types and max size as needed
    //     ]);
    //     //dd($data);
    //     $fileName = time() . '_' . $request->file('image')->getClientOriginalName();
    //     $path = $request->file('image')->storeAs('uploads', $fileName, 'public');

    //     // Save the details in the database using DB query
    //     DB::table('login_page_image')->insert([
    ...
    //     return redirect('superadmin/login_page_file');
    // }
    
    public function upload(Request $request, $id = null)
    {
        
        // Validate input, making image nullable for edits
        $data = $request->validate([
            'name' => 'required|string|max:255',
            'description' => 'required|string|max:255',
            'slide_order' => 'required|numeric|max:25',
            'image' => 'nullable|file|mimes:jpeg,jpg,png,gif,mp4|max:51200',
        ]);
        //dd($data);
        $path = null;
        ...
        if ($id) {
            ...
            DB::table('login_page_image')->where('id', $id)->update($updateData);
            Session::flash('message', 'Upload Successfully Updated.');
        } else {
            // Insert new record
            DB::table('login_page_image')->insert([
                'file_name' => $request->input('name'),
    ...
    public function edit($id)
{
    ...
    $video = DB::table('login_page_image')->where('id', $id)->first();
    ...
    // Return the edit view with the video data
    return view('admin/login_file_form', compact('video')); // Replace 'your.edit.view.name' with your actual view name
}
    ...
    public function view_register_file_dtl(Request $request)
    {
        if (!empty(Session::get('empsu_email'))) {
            // Get the latest video (change this logic as per your needs)
            $videos = DB::table('register_page_image')->get();
        
            if ($videos->isNotEmpty()) {
                // Pass all videos to the view
                return view('admin/register_file', compact('videos'));
            } else {
                // Handle case when no video is found
                Session::flash('message', 'No videos found');
                return view('admin/register_file', compact('videos'));
    ...
    public function view_register_file_form(Request $request)
    {
        if (!empty(Session::get('empsu_email'))) {
            return view('admin/register_file_form');
        } else{
            return redirect('superadmin');
        }  
    }

    // public function uplode_image(Request $request)
    // {
    //   // dd($request->all());
    //     $data = $request->validate([
    //         'name' => 'required|string|max:255',
    ...
    //     // Save the details in the database using DB query
    //     DB::table('register_page_image')->insert([
    //         'file_name' => $request->input('name'),
    //         'description'=> $request->input('description'),
    //         'file_path' => $path,
    //         'created_at' => now(), // Add created_at timestamp
    ...
    //     Session::flash('message', 'Upload Successfully Saved.');
    //     return redirect('superadmin/registration_page_file');
    // }
    
        public function uplode_image(Request $request, $id = null)
    {
        //dd('ok');
        // Validate input, making image nullable for edits
        $data = $request->validate([
        ...
        if ($id) {
            ...
            DB::table('register_page_image')->where('id', $id)->update($updateData);
            Session::flash('message', 'Upload Successfully Updated.');
        } else {
            // Insert new record
            DB::table('register_page_image')->insert([
    ...
    public function registeredit($id)
    {
        //dd('okk');
        // Fetch the video data from the database
        $video = DB::table('register_page_image')->where('id', $id)->first();
        ...
        // Return the edit view with the video data
        return view('admin/register_file_form', compact('video')); // Replace 'your.edit.view.name' with your actual view name
    }
    ...
    public function deleteVideo($id)
    {
        ...
        //dd($video);
        if ($video) {
            // Define the file path in the storage folder (relative to storage/app/public)
            $filePath = 'public/' . $video->file_path;
            ...
            DB::table('register_page_image')->where('id', $id)->delete();
    
            // Redirect back with a success message
            return redirect()->back()->with('success', 'Video and record deleted successfully.');
        } else {
            return redirect()->back()->with('error', 'Video not found.');
        }
    }
    ...
    public function deleteLoginVideo($id)
    {
        // Fetch the video record by ID
        $video = DB::table('login_page_image')->where('id', $id)->first();
        //dd($video);
        if ($video) {
            ...
            // Delete the video record from the database
            DB::table('login_page_image')->where('id', $id)->delete();
        ...
        try {
            ...
            if (!empty($email)) {
                ...
                $data['tax_rs'] = DB::Table('tax_bill')
                    ->where('status', '=', 'active')
                    ->get();
                    $data['items'] = BillingItem::all();    
                if($userType=='sub-admin'){
                    return view('sub-admin/billing/billing-add-new', $data);
                }   
                //dd($data); 
                return View('admin/invoice/billing-add-new2', $data);
    ...
    //     $allData = DB::table('registration')->where ('status', 'active')->where('verify', 'approved')->get();
    //     dd($allData);
    ...
    //     // Get all active and approved organizations
    //     $organizations = DB::table('registration')
    //         ->where('status', 'active')
    //         ->where('verify', 'approved')
    //         ->get();
    ...
    //             'org_name' => $org->f_name . ' '.$org->l_name. ' ' . $org->l_name,
    //             'org_email' => $org->email,
    ...
    //                     'sub_name' => $subAdmin->f_name . ' '.$subAdmin->l_name. ' ' . $subAdmin->l_name,
    ...
    //     // Apply default filters for active and approved organizations
    ...
    //             // ->orWhere('verify', 'like', '%'.$searchTerm.'%');
    //         });
    //     }
    ...
    //         ]);
    //     //return view('admin/verifycompany', compact($result));
    //     // dd($result);
    //     // return $result;
    // }
